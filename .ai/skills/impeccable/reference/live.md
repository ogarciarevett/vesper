...
```
node .ai/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .ai/skills/impeccable/scripts/live-status.mjs
node .ai/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .ai/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .ai/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .ai/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
