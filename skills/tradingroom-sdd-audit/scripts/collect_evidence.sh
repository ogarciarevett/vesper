...
run_block "Realtime Contract Scan" bash -lc "cd '$ROOT_DIR' && rg -n 'ROOM_STATE|TRADE_EVENT|FULL_STATE|STATE_DELTA|agentId|doId' packages/types apps/agent-api apps/agent-dashboard || true"
run_block "Auth Coverage Scan" bash -lc "cd '$ROOT_DIR' && rg -n 'OPENCLAW_GATEWAY_PASSWORD|x-openclaw-gateway-password|gateway_password' apps/agent-api apps/agent-dashboard apps/agent-api/.dev.vars.example README.md || true"
run_block "Emergency Stop Coverage Scan" bash -lc "cd '$ROOT_DIR' && rg -n 'emergency-stop|performEmergencyStop|/stop' apps/agent-api/src || true"
