/**
 * The `echo` pipeline — Vesper's first pipeline and the end-to-end validator for
...
/** Allowlisted handler id referenced by the `echo` task. */
export const ECHO_HANDLER_ID = "echo";
 ...
 * to confirm it received the Vesper echo pipeline and to name itself.
 */
const DEFAULT_PROMPT =
  "You are being invoked by the Vesper echo pipeline as a runtime self-test — " +
...
/**
 * Echo pipeline handler. Resolves the prompt, sends it through the CLI adapter
 * the runtime selected for this request, and records the run.
 *
 * A zero-exit completion with empty output is recorded as `error`: for a
 * self-test, an empty response is a failed echo, not a success. (The adapter
 * already throws on a non-zero exit, so that path surfaces before `recordRun`.)
 */
export const echoHandler: TaskHandler = async (ctx) => {
...
/**
 * Manual task wiring for the `echo` pipeline. `max_duration_ms` declares an
 * explicit wall-clock guardrail at the task layer (defense-in-depth above the
 * adapter's own process timeout).
 */
export const echoTaskInput: RegisterTaskInput = {
  id: "echo",
  kind: "manual",
  schedule_expr: "",
  handler_id: ECHO_HANDLER_ID,
