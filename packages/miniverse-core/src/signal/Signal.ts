...
export interface SignalConfig {
  type: 'rest' | 'websocket' | 'mock' | 'callback';
  url?: string;
  interval?: number;
  mockData?: () => AgentStatus[];
  /** For 'callback' mode: external code pushes state via this handle */
  onRegister?: (push: SignalCallback) => void;
}
    ...
    switch (this.config.type) {
      ...
      case 'callback':
        if (this.config.onRegister) {
          this.config.onRegister((agents) => this.emit(agents));
        }
        break;
    }
  }
