...
## Markdown pipelines + flow editor (editor v2) — SHIPPED

**What shipped.** Two Omar course-corrections, one cycle. (1) A pipeline IS a markdown document:
hand-rolled md ⟷ doc converter (fail-closed, line-numbered errors, round-trip property tested);
the `~/.vesper/pipelines/*.md` drop folder (filename = id, swept at boot + `vesper pipeline sync`,
unchanged files skip the revision bump); `vesper pipeline new|edit|sync`, markdown-default export,
md-aware save. (2) The editor became a drag-and-drop DAG canvas on **Drawflow** (the second
authorized runtime dependency — 64 KB, zero transitive deps; React Flow rejected at ~8 MB +
framework): palette drag-in, compact nodes, cycle-refusing edge connect, single-node inspector,
Canvas/Markdown view toggle. Doc v1.1 added `layout` + `after` + `{{steps.<id>.result}}`; graph.ts
levels the DAG back into the SAME staged interpreter (no second engine). (3) Built-ins expose
their REAL prompts (prompt catalogs built by calling the actual builders with placeholders) — the
empty-template box and the black-box complaint are gone; the built-in Run form now asks the
contract's real params. Verified live: a hand-written morning-note.md synced, registered, and ran
with id-keyed piping.

**Deltas & lessons.**
- *Three parallel sub-agents worked cleanly* (md converter, prompt catalogs, lead on canvas +
  core) because slices were file-disjoint and each brief named exact files, exact API signatures,
  and gates. The one shared file (pipelines/index.ts) was integrated by the lead. The Foundation
  pattern still holds at this scale.
- *The graph didn't need a new runtime.* Edges -> topological levels -> the existing stages was a
  ~120-line pure module; the canvas is presentation only. Resisting a "graph executor" kept the
  anti-openclaw line intact.
- *Client/server algorithm duplication is acceptable when tiny and tested*: `levelGraph` is
  restated in the client (~30 lines, own test) because the browser bundle cannot import
  @vesper/pipelines; the server version remains the source of truth at save time.
- *Auto-generated docs bite hand-editors*: the README command table is generated by `docs:cli`;
  hand-edits there were overwritten — regenerate, don't edit.

**UI gate.** Impeccable audit + browser verification on the canvas: one P0 (the canvas collapsed
to 2px — Drawflow adds its `parent-drawflow` class to the host element, and the injected
`height:100%` rule beat the host's `520px` at equal specificity; happy-dom tests cannot catch
layout), two P1s (nodes were mouse-only; the shared toast was silent to screen readers), and P2
quick wins (Space on palette items, staggered keyboard adds, canvas aria-label, layout-validator
double error). ALL fixed and re-verified 5/5 PASS in a real browser with zero console errors.
Lesson: a third-party widget that mutates YOUR element's classes can flip your own CSS — pin the
combined selector (`.pf-canvas-host.parent-drawflow`).

**Follow-ups**: canvas keyboard-only edge wiring; undo; minimap (only if pipelines grow);
file-watch for the drop folder (v1 is boot + explicit sync); editable built-in prompt OVERRIDES
(the read-only catalogs are the prerequisite, the override needs its own safety pass).

---

## Addendum (same cycle, pre-commit): editor UX distill + Loop relocation — 2026-06-10

**What changed.** Omar reviewed the shipped editor live and course-corrected twice before the
commit: (1) the standalone Loop sidebar section "no tiene sentido" — autonomy belongs in pipeline
creation; (2) the area below the canvas was "muy complicada" — simplify with the impeccable skill.
Ran `$impeccable critique` (two independent sub-agents: design review + deterministic detector with
live-browser evidence; snapshot persisted at `.impeccable/critique/`, score 20/40, 1 P0 + 3 P1).
Fixes shipped: Loop section removed (14 sections; `POST /api/loop/run` + `vesper loop` untouched);
the loop became the FEATURED "Autonomous step" palette entry (plain-language inspector — goal,
"Done when (optional)", round cap with live "up to ~N calls to your AI helper" projection, the
can-only-think constraint — all ported from the old section) plus a "Start autonomous" launcher
beside New pipeline; Save moved to the top bar and disabled while invalid, with validation errors
in a `role="alert"` strip beside the canvas whose entries click-select the offending step; the
inspector's 16 always-open skill chips + command + cli/model collapsed behind one "Advanced —
default routing" disclosure (live summary); Delete/Cross-share demoted to a quiet bottom border
row; Improve audit scrolls into view; AA contrast (`--accent-strong: #754ffb` for filled controls
= 4.92:1 white text, `::placeholder` pinned to `--ink-soft`, pf-hint 11.5 → 12.5px).

**Gates.** 1445/0 tests, Biome exit 0, tsc 0 errors, browser re-verification 10/10 PASS with an
empty console.

**Deltas & lessons.**
- *A section that only STARTS something is a smell* — the Loop section's one real asset was its
  explanatory copy + cost line; relocating those INTO the creation affordance made autonomy more
  discoverable, not less. The derived-capability cards now also cover the autonomous step for free.
- *The critique's working-memory count (33 controls below the canvas) was the argument that made
  every individual fix obvious*; cognitive-load arithmetic beats taste debates.
- *Gate validity before ceremony*: Save-clickable-while-invalid made users pay the approval-code
  round-trip for a doc the daemon would reject — order the gates by cost.

**Follow-ups (Omar's call).**
- Approval ceremony fires on EVERY save regardless of capability delta — require the code only on
  first save / capability GROWTH? (security-policy change, not taken unilaterally).
- Per-step cli/model selects in the v1 UI at all? The orchestrator + benchmarks already route.
- The dirty-leave guard is a native `window.confirm` — an in-app dialog would match the dark glass
  and be automatable.

---

## Addendum 2 (same cycle, pre-commit): model picker + live model directory — 2026-06-10

**What changed.** Omar: "The selection of the LLM is terrible — should show the icon of the company
and the list", pointing at OpenRouter's catalog as the source (the way the AI-SDK ecosystem reads
it) and ComfyUI as the interaction reference. Shipped: (1) `vesper-core/models/directory.ts` — the
live directory from OpenRouter's PUBLIC `/api/v1/models` (no API key) via the shipped
`allowlistedFetch` pinned to `openrouter.ai`; rows filtered to what the user's CLIs can actually
run (anthropic→claude with dots→dashes flags, openai gpt-*→codex, google gemini-*→gemini;
`:free`/image/gemma/lyria variants dropped), newest first, deduped against the curated catalog.
(2) `GET /api/models/directory` — daemon-cached (1h TTL, shared in-flight fetch), fail-soft
(`{available:false}` on any failure; the picker degrades to the catalog). (3) `inferModelCli` in
the cli-resolver — raw directory ids route to the right adapter by id shape, ONLY when neither the
catalog nor an explicit `opts.cli` chose one (an opencode override running a claude model is never
second-guessed; an inferred CLI that is not installed drops the flag — routing never kills a run).
(4) `shell/model-picker.ts` — dependency-free searchable dropdown: provider mark + name + tier or
context meta per row, grouped (Vesper catalog, Anthropic, OpenAI, Google), keyboard-complete
(type-to-filter, Enter picks the first hit, arrows, Escape), `position: fixed` so the sticky
sidebar never clips it, flips/shrinks on short viewports, close-listeners deferred one tick so the
opening click/focus-scroll can't self-dismiss it. Replaced all three editor model selects.
ComfyUI is GPL-3: the interaction pattern was the reference, zero code copied (Vesper is MIT).

**Gates.** 1461/0 tests (+16 new: directory parser/fetch, resolver inference, route fail-soft,
contextMeta), Biome exit 0, tsc 0 errors. Browser-verified live three times (full flow 8/8; short-
viewport flip + Enter-picks-first; native-CDP-click open/outside-click close) — console clean.

**Deltas & lessons.**
- *The "fetch models from the provider APIs" ask resolves to OpenRouter alone*: Anthropic/OpenAI/
  Gemini list-models endpoints all need API keys Vesper deliberately does not hold; OpenRouter's
  public catalog covers all three without one. Same Hard-rule-12 shape as benchmark-ingest: an
  allowlisted METADATA fetch, never LLM inference.
- *Routing by id shape beats registering fetched models into the catalog*: zero state, works for
  hand-typed ids in `vesper pipeline` docs too, and the explicit-override semantics stay intact.
- *A fixed-position popover needs its close-listeners deferred one tick* — the opening click's own
  scroll/focus side effects otherwise close it instantly (caught only by a NATIVE click in the
  browser; synthetic el.click() hid it).

**Follow-ups.** Picker for the chat composer / router template (same component, different host);
per-row pricing meta (OpenRouter ships prompt/completion USD); a `vesper models --live` CLI face
for the same directory.

---

## Addendum 3 (same cycle, pre-commit): voice in the chat + ElevenLabs + orchestrator pill + person-to-person tone — 2026-06-10

**What changed.** Three more Omar course-corrections. (1) **Voice lives IN the chat** (like
ChatGPT/Claude voice), not in a separate tab: the chat composer gained a feature-detected mic button
(SpeechRecognition; interim transcript streams into the input, final auto-sends) and a persisted
"Voice replies" toggle that speaks each COMPLETED assistant reply (never deltas, never backfilled
history; barge-in stops speech). The standalone Voice section was deleted (13 sections); the voice
provider became a Settings card. (2) **ElevenLabs as an opt-in cloud voice** —
`vesper-core/voice/elevenlabs.ts`: TTS over the user's OWN API key through `allowlistedFetch`
pinned to exactly `api.elevenlabs.io` (no SDK, no new dependency; brain stays the CLI — the
voice-conversation spec's planned cloud provider). Key lives in the keychain only
(`elevenlabs_api_key`); config (`voice.tts`, `voice.elevenLabsVoiceId/ModelId`) + key are read
FRESH per call so a Settings save needs no daemon restart. Routes: `POST /api/voice/tts` (mp3
bytes, or `{available:false}` so the chat falls back to the local browser voice — never an error
mid-conversation), `GET/POST /api/voice/config` (local-origin; key never echoed — `keyConfigured`
only). The client helper (`shell/speech.ts`) tries the cloud route and falls back to
`speechSynthesis`; `stripForSpeech` keeps markdown out of the spoken text. (3) **The orchestrator
shows its model top-right**: `/api/status` gained `orchestratorModel` (the SAME resolver the router
registration uses — template pin > benchmark frontier > configured default, extracted to one
function in daemon-run) and the titlebar renders it as the first pill. (4) **Person-to-person
tone**: the router's ANSWER prompt now instructs Vesper to match the owner's register — a greeting
gets a short human greeting, never an unrequested status report; runtime state stays ground truth
for real questions.

**Gates.** 1479/0 tests (+18: ElevenLabs seam 7, speech strip 9, voice routes + status pill),
Biome exit 0, tsc 0 errors. Browser-verified live 8/8 — including the tone fix end-to-end:
"hello hello" now answers "Hey hey, Omar. I'm here." (one sentence) where the pre-fix history
shows a multi-line status dump; console clean.

**Deltas & lessons.**
- *Tone bugs are prompt bugs*: the "orchestrator didn't understand" complaint was the ANSWER
  prompt mandating "concretely FROM the runtime state" for every message — grounding and register
  are separate instructions; ground truth only when asked.
- *Read config fresh at the seam, not at boot*, when a Settings save must take effect live.
- *Two parallel sub-agents (core seam / UI surface) + the lead on server+daemon* integrated
  cleanly because the route contracts were written down in the briefs before either started.

**Follow-ups.** `vesper voice say|ask` should honor the elevenlabs backend too (CLI parity);
ElevenLabs streaming TTS (lower first-audio latency) + voice picker (list voices from the API);
the cloud path is verified to the seam (key-less here) — first real key exercise is Omar's.
